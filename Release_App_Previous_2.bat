...
@echo 开始自动化发布
cd /d %~dp0
dotnet publish -p:JavaSdkDirectory="D:\Soft\Microsoft Visual Studio\Shared\Android\openjdk\jdk-17.0.14" -p:AndroidSdkDirectory="D:\Soft\Microsoft Visual Studio\Shared\Android\android-sdk" /p:DebugType=None /p:DebugSymbols=false App_Previous_2\CandySugar.MainUI\CandySugar.MainUI.csproj -f net9.0-android -c Release  -o ..\CandySugar\ReleaseApp_Previous_2 -p:AndroidKeyStore=true -p:AndroidSigningKeyStore=candy.keystore -p:AndroidSigningKeyAlias=candy -p:AndroidSigningKeyPass=123456 -p:AndroidSigningStorePass=123456
