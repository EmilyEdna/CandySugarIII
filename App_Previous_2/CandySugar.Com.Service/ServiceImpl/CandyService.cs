...
namespace CandySugar.Com.Service
{
    public class CandyService : ICandyService
    {
        ...
        {
            model.InitProperty();
            model.Hash = $"{model.Route}_{model.Cover}_{model.Title}".ToMd5();
        ...
        public async Task<Tuple<int, List<CollectModel>>> Get(string Category, int PageIndex)
        {
            var query = DbContext.Lite.Table<CollectModel>().Where(t => t.Platfrom == Category)
        ...
        {
           return await DbContext.Lite.Table<CollectModel>().ToListAsync();
        }

        public async Task Remove(string Category)
        {
            await DbContext.Lite.Table<CollectModel>().DeleteAsync(t => t.Platfrom == Category);
        }
   }
}
